...
public class SerializationXML {
    ...
    public static boolean Write(JSONObject text, Context context) {
        try {
            OutputStreamWriter outputStreamWriter = new OutputStreamWriter(context.openFileOutput("DataXML.json", 0));
            outputStreamWriter.write(text.toString());
            outputStreamWriter.close();

            return true;
        } catch (IOException e){
            Log.e("Exception In write XML", "File write failed: " + e.toString());
            return false;
        }
    }
    ...
    public static ArrayList<StudentItem> Read(Context context) {
        ...
        try {
            InputStream inputStream = context.openFileInput("DataXML.json");
            if ( inputStream != null ) {
            ...
            Log.e("SerializationXML", "File isn't found: " + e.toString());
            return null;
        } catch (IOException e) {
            Log.e("SerializationXML", "File isn't readable " + e.toString());
            return null;
